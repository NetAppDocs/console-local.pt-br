...
.Passos
. Selecione *Administração > Console*.
. Selecione *Configuração* para abrir a página de configuração do sistema.
. Na seção *Servidor de e-mail*, selecione *Configurar*.
. Selecione *Limpar campos* para limpar a configuração existente.
. Selecione *Salvar* para salvar a configuração limpa, o que remove a configuração do servidor de e-mail da implantação local do Console.
